...
async function login() {
    ...
    if (!user) {
        try {
            user = await Moralis.authenticate({ signingMessage: "Access to Banker DeFi Beta" })
            console.log(user)
            ...
            for (let i = 0; i < arraylength; i++) {
                ...
                if (nftOwners.result[i].owner_of == ethAddress) {
                    ...
                    const string = (user.get("ethAddress"));
                    $('#wallet').text(string.slice(0, 7));

                }
                else {
                    console.log("Strings do not match");
                    alert("Looks like you do not hold the Banker DeFi Beta Pass. Please make sure you're on the right wallet. If you are on the right wallet, reach out to an admin within our Discord and we can fix your issue.");

                }
            }

            const PFP = (user.get('Settings'));
            const black = 'black'
            if (PFP == black) {
                console.log("black pfp");
